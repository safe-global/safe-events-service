...
- **Connection reuse is configured, not default.** undici caches no DNS and defaults to a 4s keep-alive and an uncapped pool, so out of the box most deliveries pay a fresh DNS + TCP + TLS connect. `createWebhookAgent` composes `interceptors.dns()` and sets `keepAliveTimeout` and `clientTtl` (all env-tunable). The pool is left uncapped: concurrency per host is already bounded by `AMQP_PREFETCH_MESSAGES`, and capping would queue requests behind a slow host on top of the inline retry chain. Note the DNS interceptor keys pools by resolved IP, so hosts sharing an IP share a pool and undici reconnects whenever the SNI servername changes between requests on the same connection.
